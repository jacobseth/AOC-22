    ...
    if let Ok(lines) = read_lines("./work_detail") {
        ...
        for line in lines {
            if let Ok(pair_assignment) = line {
                ...
                if parsed_assignment[0][0] <= parsed_assignment[1][0] && parsed_assignment[0][1] >= parsed_assignment[1][1] {
                    println!("{}", pair_assignment);
                    total_encapsulated_work = total_encapsulated_work + 1
                } else if parsed_assignment[1][0] <= parsed_assignment[0][0] && parsed_assignment[1][1] >= parsed_assignment[0][1] {
                    println!("{}", pair_assignment);
                    total_encapsulated_work = total_encapsulated_work + 1
                }
            }
